...
def smart_tokenize(
    tokenizer: Tokenizer | RobertaTokenizer,
    sequence: str,
) -> list[str]:
    ...
    elif isinstance(tokenizer, RobertaTokenizer) or isinstance(
...
def smart_encode(
    ...
    # used by pretrained roberta-base
        ...
        tmp = tokenizer(
            sequences_in_batch, padding="longest", is_split_into_words=False
        )
        encodings = []
...
class DataSet:
    ...
    def for_probing(
        ...
        def _get_mask_pattern_from_probing_sequence(
            sequence: str,
        ) -> tuple[int]:
            ...
            res = [
                i for i, token in enumerate(tokens) if token.endswith(Data.mask_symbol)
            ]
            return tuple(res)
        ...
        data = list(
            zip(
                sequences,
                [_get_mask_pattern_from_probing_sequence(s) for s in sequences],
            )
        )
    ...
    def __init__(
        ...
        sequences: list[str],
        tokenizer: Tokenizer | RobertaTokenizer,
        params: Params | ProbingParams,
        data: list[tuple[str, tuple[int]]] | None = None,
        ...
        assert (
            self.params.leave_unmasked_prob_start
            <= self.params.leave_unmasked_prob
            <= 1.0
        )
        ...
        self.tokenized_sequence_lengths, self.sequences = (
            self._get_tokenized_sequence_lengths()
        )
        print("Done")
        ...
        if not data:
            ...
            self.data = list(
                self._gen_sequences_and_mask_patterns()
            )  # list of sequences, each with a mask pattern
            print("Done")
            ...
            if not self.params.consecutive_masking:
                print(
                    "WARNING: Not using consecutive masking. Training data order is ignored."
                )
        ...
        if self.params.sample_with_replacement:
            self.num_batches = (
                len(self.data) // self.params.batch_size
            )  # may be slightly smaller than quantity below
        else:
            self.num_batches = len(
                list(range(0, len(self.data), self.params.batch_size))
            )

        # make curriculum for unmasking by increasing with each batch
        self.leave_unmasked_probabilities = iter(
            np.linspace(
                params.leave_unmasked_prob_start,
                params.leave_unmasked_prob,
                self.num_batches,
            )
        )

    def _gen_make_mask_patterns(
        self,
        ...
        all_mask_patterns = list(
            combinations(range(num_tokens_after_truncation), pattern_size)
        )
        ...
        while num_yielded < num_patterns:
            if self.params.probabilistic_masking:
                if self.params.mask_probability == "auto":
                    ...
                    isinstance(self.params.mask_probability, float)
                    and 0 < self.params.mask_probability < 1
                ):
                ...
                mask_pattern = tuple(
                    [
                        i
                        for i in range(num_tokens_after_truncation)
    ...
    def _get_tokenized_sequence_lengths(self):
        ...
        for s in self._sequences:
            ...
            if self.disallow_sub_words_when_probing and isinstance(
                self.params, ProbingParams
            ):
            ...
            if (
                not self.params.allow_truncated_sentences
                and num_tokens_and_special_symbols > self.params.max_input_length
            ):
                num_too_large += 1
                continue

            num_tokens_total += num_tokens
            num_tokens_after_truncation = min(
                self.params.max_input_length - 2,
                # -2 because we need to fit eos and bos symbols
                num_tokens,
            )  # prevent masking of token in overflow region
            tokenized_sequence_lengths.append(num_tokens_after_truncation)
            sequences.append(s)

        if self.params.allow_truncated_sentences:
            print("Did not exclude sentences because truncated sentences are allowed.")
        else:
            print(
                f"Excluded {num_too_large} sequences with more than {self.params.max_input_length} tokens."
            )
        print(
            f"Mean number of tokens in sequence={num_tokens_total / len(sequences):.2f}",
            flush=True,
        )

        return tokenized_sequence_lengths, sequences

    def _gen_sequences_and_mask_patterns(
        self,
    ) -> Generator[tuple[str, tuple[int]], None, None]:
        pbar = pyprind.ProgBar(len(self.sequences))
        for s, num_tokens_after_truncation in zip(
            self.sequences, self.tokenized_sequence_lengths
        ):
            for mp in self._gen_make_mask_patterns(num_tokens_after_truncation):
                yield s, mp
            pbar.update()

    def _gen_data_chunks(
        self,
        ...
        if self.params.sample_with_replacement:
            start_ids = np.random.randint(
                0,
                num_data - self.params.batch_size,
                size=num_data // self.params.batch_size,
            )
        else:
    ...
    def mask_input_ids(
        ...
        mask_patterns: list[tuple[int]],
    ) -> tuple[dict[str, torch.tensor], torch.LongTensor | None, torch.tensor]:
        ...
        if batch_shape[1] > self.params.max_input_length:
            raise ValueError(
                f"Batch dim 1 ({batch_shape[1]}) is larger than {self.params.max_input_length}"
            )
        ...
        if unmask is not None:
            mask_insertion_matrix = (
                mask ^ unmask
            )  # XOR: True in mask will make True in mask_insertion False
        else:
        ...
        if rand_mask is not None:
            ...
            if num_rand > 0:
                input_ids[rand_mask] = np.random.choice(
    ...
    def __iter__(
        ...
        for sequences_in_batch, mask_patterns in self._gen_data_chunks():
            ...
            batch_encoding: list[Encoding] = smart_encode(
                self.tokenizer, sequences_in_batch
            )
