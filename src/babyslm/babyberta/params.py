...
param2requests = {
    ...
    "leave_unmasked_prob": [
        0.0
    ],  # WARNING: change both leave_unmasked_prob and leave_unmasked_prob_start
    ...
    "training_order": [
        "original",
        "reversed",
    ],  # original or shuffled, use this alongside consecutive_masking=True
    ...
    if "num_epochs" in param2requests:
        for num_epochs in param2requests["num_epochs"]:
            if num_epochs != 1:
                ...
                raise ValueError(
                    "Using more than one epoch is not compatible with leave_unmasked_prob curriculum."
                )
